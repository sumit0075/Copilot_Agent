...
def primeFactors(n):
    ...
    while i * i <= n:  # Only need to check up to sqrt(n)
        whi n % i == 0:  # While i divides n completely
            if i not in factors:
    ...
    if n > 1:  # If remaining n is greater than 1, it is a prime factor
        factors[n] = 0  # Increment exponent of prime factor (with default value)
...
def factorial(n):
    relt = 1  # Initialize result
    for i in range(2, n + 1):  # Iterate from 2 to n
        factors = primeFactors(i)  # Get prime factors of i
        for p in factors:  # Use 'factors' variable defined earlier
            result *= math.prod(factors[p])  # Compute factorial using prime factorization (with math.prod())
